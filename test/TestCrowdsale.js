var DemoCoinCrowdsale = artifacts.require("DemoCoinCrowdsale");
var DemoCoin = artifacts.require("DemoCoin");

contract('DemoCoinCrowdsale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        DemoCoinCrowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 1000 DemoCoin Tokens in PreICO', function(done){
        DemoCoinCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const demoCoin = DemoCoin.at(tokenAddress);
            const tokenAmount = await demoCoin.balanceOf(accounts[7]);
            assert.equal( .toNumber(), 5000000000000000000, 'The sender didn\'t receive the tokens as per PreICO rate');
            done();
       });
    });
    ...
    it('should transfer the ETH to wallet immediately in Pre ICO', function(done){
        DemoCoinCrowdsale.deployed().then(async function(instance) {
            ...
            console.log('balanceOfBeneficiary=' + balanceOfBeneficiary);
            ...
            console.log('newBalanceOfBeneficiary=' + newBalanceOfBeneficiary);
    ...
    it('one ETH should buy 2 DemoCoin Tokens in ICO', function(done){
        DemoCoinCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const demoCoin = DemoCoin.at(tokenAddress);
            const tokenAmount = await demoCoin.balanceOf(accounts[2]);
